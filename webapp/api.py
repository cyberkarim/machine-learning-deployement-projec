...
if uploaded_image is not None:
    ...
    uploaded_image_save_path="data/prod images/"+uploaded_image.name
    with open(uploaded_image_save_path, "wb") as f:
        f.write(uploaded_image.read())

    uploaded_image={
        #"image file name":uploaded_image_save_path,
        "link":uploaded_image_save_path
    }
    response = requests.post(url="http://172.19.0.2:8000/predict", json=uploaded_image)
...
if st.button("Soumettre"):
    ...
    response=requests.post("http://172.19.0.2:8000/feedback", json=feedback_image)
